...
namespace WindowsFormsApp
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT ordedrid, odate, O.pid, P.pname, O.cid, C.cname, qtyt,price,total FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT( ordedrid, odate, O.pid, P.pname, O.cid, C.cname, qtyt) LIKE '%"+textSearch.Text+"%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvOrder_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbOrder WHERE ordedrid LIKE '" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Order has been deleted.");

                    cm = new SqlCommand("UPDATE tbProduct SET pqtyt = (pqtyt + @pqty) WHERE pid LIKE '" + dgvOrder.Rows[e.RowIndex].Cells[3].Value.ToString() + "' ", con);
